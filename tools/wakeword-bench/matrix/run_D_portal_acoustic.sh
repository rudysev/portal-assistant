...
#
# Arms assistant with files/wakebench, plays at ACOUSTIC_VOLUME (100), scores oww-score peaks.
#
# Usage:
...
LIST_ABS="$(mktemp /tmp/matrix_D.XXXXXX)"
...
while IFS='|' read -r path category condition <&3; do
  ...
  base=$("$ADB" shell "wc -l < $DBG" </dev/null | tr -d '\r ' || echo 0)
  afplay "$path" </dev/null
  sleep 1.6
  new=$("$ADB" shell "tail -n +$((base+1)) $DBG" </dev/null || true)
  peak=$(echo "$new" | grep -oE 'oww-score jarvis [0-9.]+' | awk '{print $3}' | sort -rn | head -1 || true)
  [ -z "$peak" ] && peak=0.000
  prms=$(echo "$new" | grep -oE 'rms=[0-9]+' | sed 's/rms=//' | sort -rn | head -1 || true)
  [ -z "$prms" ] && prms=0
