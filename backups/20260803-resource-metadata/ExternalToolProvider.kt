...
class ExternalToolProvider(context: Context) {
    ...
    companion object {
            ...
            buildList {
                for (pi in pkgs) {
                    for (prov in pi.providers ?: continue) {
                        val json = metadataString(pm, pi.packageName, prov.metaData, ToolContract.META_DECLARATIONS) ?: continue
                        val authority = prov.authority ?: continue
                        val summary = metadataString(pm, pi.packageName, prov.metaData, ToolContract.META_SUMMARY)
        ...
        /** Android 9 can expose resource-backed provider metadata as an integer resource id. */
        private fun metadataString(pm: PackageManager, pkg: String, data: Bundle?, key: String): String? {
            val value = data?.get(key) ?: return null
            return when (value) {
                is String -> value
                is Int -> runCatching { pm.getResourcesForApplication(pkg).getString(value) }.getOrNull()
                else -> null
            }
        }
